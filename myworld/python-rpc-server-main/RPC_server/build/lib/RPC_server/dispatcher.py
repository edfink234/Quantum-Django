...
from RPC_server.RPC_server.JsonInterfaceGeneration import JSONInterfaceGeneration as jig
...
class Dispatcher(MutableMapping):
    ...
    def add_class(self, cls: Any, prefix: Optional[str] = None) -> None:
        ...
        # add json interface to rpc_info_list
        self.rpc_info_list.append(jig.generate(cls))
    ...
    def add_function(self, f: Callable = None, name: Optional[str] = None) -> Callable:
        ...
        # add json interface to rpc_info_list
        if f.__name__ != 'rpc_info':
            self.rpc_info_list.append(jig.generate(f))
        

        return f
    
    def rpc_info(self):
        return self.rpc_info_list
